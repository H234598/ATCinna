...
- Rich audio-list actions: ATPlayer has table/context-menu workflows such as metadata/info dialogs and broader audio actions; ATCinna now covers compact popup info with **Filminformation anzeigen** alias, visible result selection with batch play/save, direct **Abspielen**/**Speichern** context actions, audio URL/title/genre/topic copy actions, first Bookmark add/remove/clear workflows, and first seen/unseen history actions, but still lacks deeper table-level audio workflows.
