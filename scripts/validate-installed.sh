...
if ! rg -q -F 'this.settings.bind("system-color-theme-1", "systemColorTheme1", this._onColorThemeChanged.bind(this));' "$APPLET_JS"; then
    echo "ERROR: installed applet does not bind system-color-theme-1"
    exit 1
fi
...
if ! rg -q -F '"system-color-theme-1": false' "$APPLET_JS"; then
    echo "ERROR: installed applet reset defaults do not restore system-color-theme-1"
    exit 1
fi
...
if ! rg -q -F 'this.settings.setValue("system-color-theme-1", defaults["system-color-theme-1"]);' "$APPLET_JS"; then
    echo "ERROR: installed applet reset does not persist system-color-theme-1"
    exit 1
fi
...
if ! rg -q -F 'new PopupMenu.PopupSwitchMenuItem("Farb-Modus-1"' "$APPLET_JS"; then
    echo "ERROR: installed applet does not contain Farb-Modus-1 switch"
    exit 1
fi
...
if ! rg -q -F '_onColorThemeChanged' "$APPLET_JS"; then
    echo "ERROR: installed applet is missing _onColorThemeChanged handler"
    exit 1
fi
if ! rg -q -F '_applyColorTheme()' "$APPLET_JS"; then
    echo "ERROR: installed applet is missing _applyColorTheme handler"
    exit 1
fi
if ! rg -q -F 'const nextValue = this._boolSetting(state, false)' "$APPLET_JS"; then
    echo "ERROR: installed applet switch handlers do not use toggled state"
    exit 1
fi
if ! rg -q -F 'add_style_class_name("atcinna-color-mode-1")' "$APPLET_JS"; then
    echo "ERROR: installed applet does not apply atcinna-color-mode-1 class"
    exit 1
fi
if ! rg -q -F 'remove_style_class_name("atcinna-color-mode-1")' "$APPLET_JS"; then
    echo "ERROR: installed applet does not clear atcinna-color-mode-1 class"
    exit 1
fi
...
if ! rg -q -F '.atcinna-color-mode-1' "$APPLET_DIR/stylesheet.css"; then
    echo "ERROR: installed stylesheet is missing atcinna-color-mode-1 class"
    exit 1
fi
...
checks = {
    ...
    "filter profile clear label is created": re.compile(
        r"this\._clearFilterProfilesItem\s*=\s*new\s+PopupMenu\.PopupMenuItem\s*\(\s*['\"]Alle Filterprofile löschen['\"]\s*\)",
        re.S,
    ),
...
for applet_label in "Neue Bookmarks anlegen" "Bookmarks löschen" "Alle angelegten Bookmarks löschen"; do
    if ! rg -q -F "${applet_label}" "$APPLET_JS"; then
        echo "ERROR: installed applet label is missing: ${applet_label}"
        exit 1
    fi
done
for applet_bookmark_ui_label in "nur Bookmarks" "Nur Bookmarks" "Bookmarks" "keine Bookmarks"; do
    if ! rg -q -F "${applet_bookmark_ui_label}" "$APPLET_JS"; then
        echo "ERROR: installed applet bookmark UI wording is missing: ${applet_bookmark_ui_label}"
        exit 1
    fi
done
for applet_bookmark_ui_contract in \
    ...
    if ! rg -q -F "${applet_bookmark_ui_contract}" "$APPLET_JS"; then
        echo "ERROR: installed applet bookmark UI wording contract is missing: ${applet_bookmark_ui_contract}"
        exit 1
    fi
done
for legacy_bookmark_ui_label in "nur Favoriten" "Nur Favoriten" "Zu Favoriten hinzufügen" 'PopupMenu.PopupMenuItem("Favoriten")' "keine Favoriten"; do
    if rg -q -F "${legacy_bookmark_ui_label}" "$APPLET_JS"; then
        echo "ERROR: installed applet legacy favorite wording remains in visible bookmark UI: ${legacy_bookmark_ui_label}"
        exit 1
    fi
done
...
for applet_queue_download_submenu in \
    ...
    if ! rg -q -F "${applet_queue_download_submenu}" "$APPLET_JS"; then
        echo "ERROR: installed applet queue entry downloads submenu wiring is missing: ${applet_queue_download_submenu}"
        exit 1
    fi
done
...
for applet_top_action in 'const playSelected = new PopupMenu.PopupMenuItem("Audio (URL) abspielen");' 'const playStoredSelected = new PopupMenu.PopupMenuItem("Gespeichertes Audio (Datei) abspielen");' 'const editSelected = new PopupMenu.PopupMenuItem("Download ändern");' 'const copySelected = new PopupMenu.PopupMenuItem("Download (URL) kopieren");' 'const infoSelected = new PopupMenu.PopupMenuItem("Audioinformation anzeigen");'; do
...
for applet_symbol in "_runQueuePlaySelected" "_runQueueOpenStoredSelected" "_runQueueEditSelected" "_runQueueCopySelected" "_runQueueShowInfoFirstSelected" "_runQueueRunSelected" "_runQueuePreferSelected" "_runQueuePutBackSelected" "_queueActionRunSelected" "_queueActionPlaySelected" "_queueActionPlayStoredSelected" "_queueActionEditSelected" "_queueActionCopySelected" "_queueActionShowInfoSelected" "_queueActionPreferSelected" "_queueActionPutBackSelected"; do
