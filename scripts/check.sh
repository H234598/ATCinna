...
for queue_label in "Alles auswählen" "Auswahl umkehren" "Tabelle zurücksetzen" "Downloads starten" "Audio (URL) abspielen" "Audioinformation anzeigen" "Download ändern" "Download (URL) kopieren" "Downloads vorziehen" "Downloads zurückstellen" "Downloads stoppen" "Downloads aus Liste entfernen" "Auswahl umschalten"; do
    if ! rg -q -F "${queue_label}" "$APPLET_JS"; then
        echo "ERROR: applet queue selection label is missing: ${queue_label}"
        STATUS=1
    fi
done
for queue_selected_action in 'const runSelected = new PopupMenu.PopupMenuItem("Downloads starten");' 'const playStoredSelected = new PopupMenu.PopupMenuItem("Gespeichertes Audio (Datei) abspielen");' 'const editSelected = new PopupMenu.PopupMenuItem("Download ändern");' 'const copySelected = new PopupMenu.PopupMenuItem("Download (URL) kopieren");' 'const infoSelected = new PopupMenu.PopupMenuItem("Audioinformation anzeigen");' 'const preferSelected = new PopupMenu.PopupMenuItem("Downloads vorziehen");' 'const putBackSelected = new PopupMenu.PopupMenuItem("Downloads zurückstellen");' 'const cancelSelected = new PopupMenu.PopupMenuItem("Downloads stoppen");' 'const removeSelected = new PopupMenu.PopupMenuItem("Downloads aus Liste entfernen");'; do
...
for queue_top_action in 'const playSelected = new PopupMenu.PopupMenuItem("Audio (URL) abspielen");' 'const playStoredSelected = new PopupMenu.PopupMenuItem("Gespeichertes Audio (Datei) abspielen");' 'const editSelected = new PopupMenu.PopupMenuItem("Download ändern");' 'const copySelected = new PopupMenu.PopupMenuItem("Download (URL) kopieren");' 'const infoSelected = new PopupMenu.PopupMenuItem("Audioinformation anzeigen");'; do
...
for queue_selection_handler in "_queueSelectionItems" "_queueItemsCache" "_queueActionRunSelected" "_queueActionPlaySelected" "_queueActionPlayStoredSelected" "_queueActionEditSelected" "_queueActionCopySelected" "_queueActionShowInfoSelected" "_queueActionPreferSelected" "_queueActionPutBackSelected" "_runQueueSelectAll" "_runQueueInvertSelection" "_runQueueResetSelection" "_runQueueToggleSelection" "_runQueueRunSelected" "_runQueuePlaySelected" "_runQueueOpenStoredSelected" "_runQueueEditSelected" "_runQueueCopySelected" "_runQueueShowInfoFirstSelected" "_runQueuePreferSelected" "_runQueuePutBackSelected" "_runQueueCancelSelected" "_runQueueRemoveSelected" "_runQueueBatchAction" "_getSelectedQueueItems" "_queueItemKey" "_updateQueueSelectionActionState"; do
...
if rg -q -F '["Dauer", safeItem.duration]' "$APPLET_JS"; then
    echo "ERROR: old info section duration label is still present"
    STATUS=1
fi
